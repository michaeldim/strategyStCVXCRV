...
if command -v slither &> /dev/null; then
  ...
  echo -e "${YELLOW}Running Slither on CvxCrvCompounder.sol...${NC}"

  # Remove any existing slither output to avoid the prevention of overwrite
  rm -f /tmp/slither_results.json

  # Run on CvxCrvCompounder.sol
  slither src/CvxCrvCompounder.sol --json /tmp/slither_results.json > /tmp/slither_check.log 2>&1
  SLITHER_EXIT_CODE=$?
  ...
  if [ -f /tmp/slither_results.json ]; then
    ...
    if [ "$HIGH_FINDINGS" -gt 0 ]; then
      echo -e "${RED}✗ Slither found $HIGH_FINDINGS high severity issues in CvxCrvCompounder.sol${NC}"
      SLITHER_FAILED=true
    elif [ $SLITHER_EXIT_CODE -ne 0 ]; then
      echo -e "${RED}✗ Slither check on CvxCrvCompounder.sol failed with exit code $SLITHER_EXIT_CODE${NC}"
      SLITHER_FAILED=true
    else
      echo -e "${GREEN}✓ Slither check on CvxCrvCompounder.sol passed${NC}"
    fi
  else
...
if [ $build_exit -eq 0 ]; then
  ...
  grep "CvxCrvCompounder" /tmp/build_output.log | head -n 3
  ...
  if grep -q "CvxCrvCompounder" /tmp/build_output.log; then
    echo -e "${YELLOW}Contract sizes:${NC}"
    grep "CvxCrvCompounder" /tmp/build_output.log | head -n 3
  fi
else
