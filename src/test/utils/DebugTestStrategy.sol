...
contract DebugTestStrategy is StCVXCRVStrategy {
    ...
    function testHarvest() external returns (uint256) {
        console.log(
            "DebugTestStrategy: Starting testHarvest -> _harvestAndReport"
        );
        ...
        returns (uint256 _totalAssets)
    {
        ...
        if (!mockIsShutdown) {
            console.log(
                "DebugTestStrategy._harvestAndReport: Calling _doClaimRewardsFromWrapper"
            );
            _doClaimRewardsFromWrapper();
        } else {
            console.log(
                "DebugTestStrategy._harvestAndReport: mockIsShutdown is true, skipping reward claim"
            );
        }

        console.log(
            "DebugTestStrategy._harvestAndReport: Calling _doSellRewards"
        );
        _doSellRewards();
        ...
        try IERC20(CVXCRV).balanceOf(address(this)) returns (uint256 balance) {
            cvxCrvBal = balance;
            console.log(
                "DebugTestStrategy._harvestAndReport: CVXCRV balance in strategy ="
            );
            console.logUint(cvxCrvBal);
        } catch {
            console.log(
                "DebugTestStrategy._harvestAndReport: ERROR: CVXCRV.balanceOf reverted"
            );
            cvxCrvBal = 0;
        }
        ...
        if (cvxCrvBal > 0 && !mockIsShutdown) {
            console.log(
                "DebugTestStrategy._harvestAndReport: Attempting to transfer CVXCRV to WRAPPER and stake"
            );
            try IERC20(CVXCRV).transfer(address(WRAPPER), cvxCrvBal) returns (
                bool success
            ) {
                if (success) {
                    console.log(
                        "DebugTestStrategy._harvestAndReport: CVXCRV transfer to WRAPPER succeeded"
                    );
                    try WRAPPER.stake(cvxCrvBal, address(this)) {
                        console.log(
                            "DebugTestStrategy._harvestAndReport: WRAPPER.stake succeeded"
                        );
                    } catch Error(string memory reason) {
                        console.log(
                            "DebugTestStrategy._harvestAndReport: WARNING: WRAPPER.stake failed with reason:"
                        );
                        console.log(reason);
                    ...
                    console.log(
                        "DebugTestStrategy._harvestAndReport: WARNING: CVXCRV transfer to WRAPPER failed"
                    );
                }
            } catch Error(string memory reason) {
                console.log(
            ...
            console.log(
                "DebugTestStrategy._harvestAndReport: No CVXCRV to stake or mockIsShutdown is true"
            );
        }
        ...
        console.log(
            "DebugTestStrategy._harvestAndReport: Total assets calculated ="
        );
    ...
    function _doClaimRewardsFromWrapper() internal {
        console.log(
            "  _doClaimRewardsFromWrapper: Attempting WRAPPER.getReward"
        );
        try WRAPPER.getReward(address(this)) {
            console.log(
                "  _doClaimRewardsFromWrapper: WRAPPER.getReward succeeded"
            );
        } catch Error(string memory reason) {
            console.log(
                "  _doClaimRewardsFromWrapper: ERROR: WRAPPER.getReward failed with reason:"
            );
            console.log(reason);
        } catch (bytes memory reasonBytes) {
            console.log(
                "  _doClaimRewardsFromWrapper: ERROR: WRAPPER.getReward failed with unknown reason (bytes):"
            );
            console.logBytes(reasonBytes);
        }
    }
    ...
    function _doSellRewards() internal {
        ...
        for (uint256 i = 0; i < strategyRewardTokens.length; i++) {
            ...
            uint256 balance;
            try IERC20(rewardToken).balanceOf(address(this)) returns (
                uint256 bal
            ) {
                balance = bal;
                console.log("    Balance =");
            ...
            if (balance > minSell) {
                console.log(
                    "    Balance exceeds minimum, attempting to sell/auction."
                );
                if (useAuction && auction != address(0)) {
                    ...
                    try ITradeFactory(tf).enable(rewardToken, CVXCRV) {
                        ...
                        console.log(
                            "    ERROR: ITradeFactory.enable failed with reason:"
                        );
                        console.log(reason);
                    } catch (bytes memory reasonBytes) {
                        console.log(
                            "    ERROR: ITradeFactory.enable failed with unknown reason (bytes):"
                        );
                        console.logBytes(reasonBytes);
                    }
                } else {
                    console.log(
                        "    No selling mechanism (auction/trade factory) is enabled or configured."
                    );
                }
            } else {
    ...
    function _doKickAuction(address _token) internal {
        ...
        if (balanceToKick > 0) {
            console.log(
                "      Approving and transferring to auction, then kicking."
            );
            try IERC20(_token).approve(auction, balanceToKick) {
                try IERC20(_token).transfer(auction, balanceToKick) returns (
                    bool success
                ) {
                    if (success) {
                        try Auction(auction).kick(_token) returns (uint256 id) {
                            ...
                            console.log(
                                "    ERROR: Auction.kick reverted with reason:"
                            );
                            console.log(reason);
                        } catch (bytes memory reasonBytes) {
                            console.log(
                                "    ERROR: Auction.kick reverted with unknown reason (bytes):"
                            );
                            console.logBytes(reasonBytes);
                        }
                    } else {
                        console.log(
                            "      WARNING: Transfer to auction failed."
                        );
                    }
                } catch Error(string memory reason) {
                    console.log(
                        "    ERROR: IERC20.transfer to auction reverted with reason:"
                    );
                    console.log(reason);
                } catch (bytes memory reasonBytes) {
                    console.log(
                        "    ERROR: IERC20.transfer to auction reverted with unknown reason (bytes):"
                    );
                    console.logBytes(reasonBytes);
                }
            } catch Error(string memory reason) {
                console.log(
                    "    ERROR: IERC20.approve for auction reverted with reason:"
                );
                console.log(reason);
            } catch (bytes memory reasonBytes) {
                console.log(
                    "    ERROR: IERC20.approve for auction reverted with unknown reason (bytes):"
                );
                console.logBytes(reasonBytes);
            }
        } else {
